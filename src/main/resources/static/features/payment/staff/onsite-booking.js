...
import {
    supabase, requireAuth, getUserProfile, maskName,
    generateQRCode, getQRExpiredAt
...
async function init() {
    ...
    obStaffName.textContent = maskName(profile.name ?? '');
    bindEvents();
    activateStep(1);
}
...
async function submitPayment() {
    ...
    try {
        ...
        if (payMethod === 'FESTIO_PAY') {
            const { data: freshUser } = await supabase
                .from('app_user').select('balance').eq('id', selectedBuyer.id).single();
            await supabase
                .from('app_user')
