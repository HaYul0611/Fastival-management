...
// ──────────────────────────────────────
async function init() {
    // URL 파라미터에서 festival_id 추출
    const params = new URLSearchParams(window.location.search);
    state.festivalId = parseInt(params.get('festival_id')) || null;

    if (!state.festivalId) {
        showModal('행사 정보를 찾을 수 없습니다.', 'error');
        return;
    }
...
async function loadFestivalData() {
    ...
    if (error || !festival) {
        showModal('행사 정보를 불러오지 못했습니다.', 'error');
        return;
    }
    ...
    Object.entries(rowMap).forEach(([rowLabel, rowSeats]) => {
        ...
        rowSeats.forEach(seat => {
            ...
            if (seat.status === '결제완료' || seat.is_reserved) {
                btn.classList.add('seat-btn--taken');
                btn.disabled = true;
            } else if (seat.status === '가선점') {
                btn.classList.add('seat-btn--hold');
                btn.disabled = true;
            } else if (isSelected) {
...
function onSeatClick(seat, btn) {
    ...
    if (existingIndex !== -1) {
        ...
        if (state.selectedSeats.length >= MAX_TICKETS) {
            DOM.maxTicketModal.hidden = false;
            return;
        }
        // 선택 추가
...
async function handleBooking() {
    // 비로그인 인터셉트
    const user = await requireAuth(() => openLoginModal());
    if (!user) return;
    ...
    for (let i = 0; i < state.selectedSeats.length; i++) {
        ...
        if (type?.type === 'VULNERABLE') {
            if (!type.name?.trim() || !type.birth?.trim()) {
                showModal(`${i + 1}번 티켓의 동반자 실명과 생년월일을 입력해주세요.`, 'warning');
                return;
            }
        }
    }
    ...
    if (paymentMethod === 'FESTIO_PAY') {
        ...
        if (profile.balance < finalPrice) {
            showModal(`FESTIO Pay 잔액이 부족합니다.\n현재 잔액: ${profile.balance.toLocaleString()}원`, 'error');
            return;
        }
...
function triggerPGPayment(user, amount, method) {
    ...
    IMP.request_pay({
        ...
        if (rsp.success) {
            ...
            showModal(`결제 실패: ${rsp.error_msg}`, 'error');
        }
    });
}
...
async function processPayment(user, paymentMethod, finalPrice) {
    try {
        ...
        showModal(`예매가 완료되었습니다!\n${state.selectedSeats.length}매의 티켓이 발급되었습니다.`, 'info');

        // 마이페이지 QR 뷰어로 이동
        setTimeout(() => {
            window.location.href = `../payment/qr-view.html?order_id=${order.id}`;
        }, 1500);

    } catch (err) {
        showModal(`결제 처리 중 오류가 발생했습니다: ${err.message}`, 'error');
        ...
        .on('postgres_changes',
            ...
            (payload) => {
                ...
                if (updated.status === '결제완료' || updated.is_reserved) {
                    ...
                    btn.classList.add(isSelected ? 'seat-btn--selected' : 'seat-btn--available');
                    btn.disabled = false;
                }
            }
        )
...
function setupEventListeners() {
    ...
    // 4매 초과 모달 닫기
    document.getElementById('maxTicketModalClose')
        .addEventListener('click', () => { DOM.maxTicketModal.hidden = true; });
    document.getElementById('maxTicketModalBackdrop')
        .addEventListener('click', () => { DOM.maxTicketModal.hidden = true; });

    // 성인 모달
    DOM.adultModalClose.addEventListener('click', () => { DOM.adultModal.hidden = true; });
    document.getElementById('adultModalBackdrop')
        .addEventListener('click', () => { DOM.adultModal.hidden = true; });
    DOM.adultModalVerifyBtn.addEventListener('click', () => {
        // PortOne 본인인증 호출 (실제 구현 필요)
        DOM.adultModal.hidden = true;
        showModal('PortOne 본인인증 모듈을 연동하세요.', 'info');
    });
    ...
    document.getElementById('loginModalBackdrop')
        .addEventListener('click', () => { DOM.loginModal.hidden = true; });
